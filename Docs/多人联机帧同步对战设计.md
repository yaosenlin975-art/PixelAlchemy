...
│   │   └── Docs/DOTS重构设计文档.md  # 注：实际路径 Client/Assets/Docs/DOTS重构设计文档.md
│   ├── HybridCLRData/
│   └── ProjectSettings/
├── Server/
│   ├── Main/                        # 服务器入口
...
> **与 DOTS重构设计文档.md 的关系**：本设计在 [DOTS重构设计文档.md](../Client/Assets/Docs/DOTS重构设计文档.md) §2 程序集划分（5 个：Core/Simulation/Renderer/Gameplay/Editor）基础上，新增 `NoitaCA.Lockstep`（帧同步调度）与 `Hotfix`（热更）两个程序集。详细玩法重构实施见 [玩法重构方案.md](./玩法重构方案.md)。
>
> **路径基准**：路径以 玩法重构方案.md §3 的 `Client/Assets/Scripts/AOT/NoitaCA/` 为准（AOT 约束），DOTS重构设计文档.md §2 的 `Assets/Scripts/NoitaCA/` 为历史路径。

```
Client/Assets/Scripts/AOT/NoitaCA/
├── NoitaCA.Core.asmdef              # DOTS 组件、共享数据（AOT）
├── NoitaCA.Simulation.asmdef        # ISystem 作业（AOT，确定性模拟）
├── NoitaCA.Lockstep.asmdef          # 帧同步调度、输入队列、状态哈希（AOT）【新增于 DOTS 重构后】
