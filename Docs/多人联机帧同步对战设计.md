...
将现有 NoitaCA 像素落沙元胞自动机改造为 **2-8 人可配置房间人数的帧同步对战游戏**（默认 4 人乱斗，房主创建房间时可设置 2-8 人，匹配模式可选 1v1/2v2/4人乱斗/8人混战等）。采用 **输入锁步 + 玩家层回滚 + 环境层周期调和（混合模型）+ 服务器中继 + 场景事件注入** 架构，集成聊天、房间、匹配、场景四大系统。

> **同步模型说明**：本设计并非"纯输入锁步"，而是分层混合模型：
> - **玩家层**（角色/法术/生物/道具）：每帧输入锁步 + 轻量级 GGPO 风格回滚（状态量 2.5KB/帧，可承受）
> - **环境层**（CA 流动像素）：周期调和（每 6 帧 200ms 服务端广播权威快照），不做实时回滚（1MB/帧回滚成本不可承受）
> - **静态层**（地形）：对局开始一次性同步
>
> 详见 [帧同步Netcode设计.md](./帧同步Netcode设计.md) §2 分层回滚模型。

### 1.1 核心决策

| 决策项 | 选定方案 | 依据 |
|--------|---------|------|
| 同步模型 | 输入锁步 + 玩家层 GGPO 回滚 + 环境层周期调和（混合模型） | 用户决策 + 帧同步Netcode设计 §2 深化 |
| 对战规模 | 2-8 人可配置（房主创建房间时设置/匹配模式 UI 选择），默认 4 人乱斗 | 用户决策 |
| 服务器权威 | 中继 + 事件注入（方案 B） | 用户决策 |
| 多线程 | 保留 DOTS 多线程，显式排序控制差异，周期调和 | 用户调整 |
| 世界规模 | 可配置竞技场，掉出边界即死亡（百战天虫规则） | 用户调整 |
| 程序集边界 | 模拟代码在 AOT，Hotfix 只放协议处理/UI/匹配请求/配置数据对象（详见 §4.1） | qq362946 建议 |
...
> **状态（2026-07-06 校对）**：以下结构为实际当前状态（`Client/Unity/` 层级已扁平化为 `Client/`，详见 §10）。

```
Noita/
├── Client/                          # Unity 客户端（已扁平化）
│   ├── Assets/
│   │   ├── Scripts/
│   │   │   ├── AOT/                 # 所有 MonoBehaviour 代码（无 asmdef，在 Assembly-CSharp）
│   │   │   │   ├── Simulation/      # 像素模拟核心
│   │   │   │   ├── Abilities/       # 法术系统
│   │   │   │   ├── Creatures/       # 生物系统
│   │   │   │   ├── Equipment/       # 装备系统
│   │   │   │   ├── Demo/            # 压测
│   │   │   │   └── ...              # PlayerController/SpellController/PixelWorldBootstrap 等
│   │   │   └── Hotfix/              # 热更程序集（仅 Hotfix.asmdef，无 .cs）
│   │   └── Docs/DOTS重构设计文档.md  # DOTS 重构设计文档（已迁入本目录）
│   ├── HybridCLRData/
│   └── ProjectSettings/
├── Server/
│   ├── Main/                        # 服务器入口
...
│       └── ExporterSettings.json    # 路径配置（已修正为 Windows 路径）
└── Docs/
    ├── 多人联机帧同步对战设计.md  # 本文档
    ├── 帧同步Netcode设计.md                  # 帧同步细化设计（本文档深化补充）
    ├── 玩法重构方案.md                 # 玩法逻辑重构方案
    └── 开发智能体配置.md                           # 项目配置
```
...
> **状态（2026-07-07 收口）**：原 4 项问题全部已修复。

1. ~~`ExporterSettings.json` 三条路径均为 macOS 默认值 `/Users/fantasy/...`，需改为 Windows 路径~~ **已修复**（见 §10.2）
2. ~~`fantasy_quickstart.md` 写 Unity 2022.3.62 LTS，实际为 6000.3.19f1~~ **已修复**（端木林，2026-07-07：第 9 行版本号已改为 `6000.3.19f1`，全文无残留）
3. ~~Fantasy-Net / NLog 包版本用 `*`，需锁定~~ **已修复**（云澈，2026-07-07：`Server/Entity/Entity.csproj` Fantasy-Net `*`→`2025.2.1402`；`Server/Main/Main.csproj` NLog `*`→`5.5.1`）
4. ~~客户端路径含 `Client/Unity/` 多余层级，需扁平化到 `Client/`~~ **已修复**（见 §10.1）
...
> **与 DOTS重构设计文档.md 的关系**：本设计在 [DOTS重构设计文档.md](./DOTS重构设计文档.md) §2 程序集划分（5 个：Core/Simulation/Renderer/Gameplay/Editor）基础上，新增 `AOT`（帧同步调度）与 `Hotfix`（热更）两个程序集。详细玩法重构实施见 [玩法重构方案.md](./玩法重构方案.md)。
>
> **路径基准**：路径以 玩法重构方案.md §3 的 `Client/Assets/Scripts/AOT/NoitaCA/` 为准（AOT 约束），DOTS重构设计文档.md §2 的 `Assets/Scripts/AOT/` 为历史路径。

```
Client/Assets/Scripts/AOT/NoitaCA/
├── AOT.asmdef              # DOTS 组件、共享数据（AOT）
├── AOT.asmdef        # ISystem 作业（AOT，确定性模拟）
├── AOT.asmdef          # 帧同步调度、输入队列、状态哈希（AOT）【新增于 DOTS 重构后】
├── AOT.asmdef          # MonoBehaviour 渲染桥接（AOT）
├── AOT.asmdef          # 玩家/法术/生物/装备（AOT）
├── Editor.asmdef            # 编辑器工具（AOT）
└── Hotfix.asmdef                    # 协议 handler、UI、房间管理（热更）【新增于 Fantasy 接入后】
```

**关键约束**（来自 qq362946 + Newman）：
- ✅ `AOT` / `AOT` 必须 AOT，**绝不在 Hotfix**
...
#### 4.2.3 周期调和（5 Hz，环境层专用）

> 注：每 6 逻辑帧 = 200ms = 5 Hz（非 10 Hz）。本机制仅作用于环境层（CA），玩家层走 GGPO 风格回滚，详见 [帧同步Netcode设计.md](./帧同步Netcode设计.md) §2。
...
| 帧同步调度 | AOT.asmdef | ❌ | AOT，避免热更 desync |
| DOTS ISystem | AOT.asmdef | ❌ | AOT，Burst 编译 |
| DOTS 组件 | AOT.asmdef | ❌ | AOT |
| 渲染桥接 | AOT.asmdef | ❌ | AOT |

**Hotfix.dll 签名校验**（来自 Newman，详见 §9.3）：
- 客户端构建时由构建机签名 Hotfix.dll 哈希（私钥不出构建机）
- 客户端启动时用内置公钥验签本地 Hotfix.dll
- 客户端连接服务器时上报 (H, signature)，服务器验签 + 白名单比对
...
```csharp
// AOT/ArenaConfig.cs
// 竞技场配置：定义世界规模、边界规则与玩家位移上限（反作弊用，详见 §9.2）
public struct ArenaConfig
{
    ...
    public Fix64 MaxSpeed;    // 玩家最大移动速度，像素/秒（用于反作弊位移检查，详见 §9.2）
}
...
#### 4.4.1 房间配置（RoomConfig）

**房间配置**（房主创建房间时设置，匹配模式下由匹配请求参数填充）：

```csharp
// Hotfix/Config/RoomConfig.cs
// 房间配置：房主创建房间时设置，匹配模式下由匹配请求参数填充
public struct RoomConfig
{
    public int PlayerCount;      // [2, 8]，默认 4，房主可设置；匹配模式由 UI 选择按钮决定
    public ArenaConfig ArenaConfig;  // 竞技场配置（含 MaxSpeed，详见 §4.4）
    // ... 其他房间配置（房间名、密码、模式等，可后续扩展）
}
```

**约束**：`PlayerCount` 必须在 [2, 8] 区间内，服务器创建房间时强制夹取（`Math.Clamp(playerCount, 2, 8)`）。

**反作弊用法**：`AntiCheatSystem.CompareHashes` 使用 `RoomConfig.PlayerCount` 计算多数派阈值（详见 §9.2）。
...
│  └── 校验: 服务器验签 (H, signature) + 白名单比对 + 资源版本号 │
├─────────────────────────────────────────────────────────┤
│  资源热更 (YooAsset)                                     │
...
```csharp
// 客户端连接服务器时上报版本
// 通过 C2G_ClientVersionReport 消息上报服务器（详见 §6.1）
public struct ClientVersionReport
{
    public string HotfixHash;          // SHA256(Hotfix.dll)
    public string HotfixSignature;     // 构建机私钥对 HotfixHash 的数字签名（详见 §9.3）
...
**服务器校验规则**：
0. 服务器验签 (H, signature) 是否有效（防伪造，详见 §9.3）
...
| **Entity** | `MatchSession` / `GameRoom` / `Player` 实体、状态哈希存储、输入验证、反作弊执行器（AntiCheatSystem，AOT） | Entity.csproj | ❌ |
| **Hotfix** | 匹配算法、房间规则、场景事件时间线、反作弊规则参数（AntiCheatConfig，含阈值与冷却数值，属 Hotfix 可调参数，详见 §4.5.2；AntiCheatSystem 使用方式见 §9.2） | Hotfix.csproj | ✅ |
...
├── GameRoom ×N (子实体)
│   ├── RoomConfigComponent    # 房间配置（含 RoomConfig.PlayerCount，由 G2C_MatchResult 下发填充）
│   ├── PlayerSlot × N          # 玩家槽位（N=RoomConfig.PlayerCount，[2, 8]）
...
> **协议完整性声明**：本节列出基础消息（输入/帧批次/场景事件/状态哈希/聊天/匹配/房间）。**重连、时钟同步、观战、网络警告等扩展消息**详见 [帧同步Netcode设计.md](./帧同步Netcode设计.md) §7 协议设计补充。
...
| `C2G_ReconnectRequest` | C→S | ProtoBuf | 一次性 | 重连请求（详见 FRAME_SYNC §7.2） |
| `G2C_ReconnectResponse` | S→C | ProtoBuf | 一次性 | 重连响应（详见 FRAME_SYNC §7.2） |
| `G2C_SnapshotChunk` | S→C | MemoryPack | 重连时分片 | 快照分片流（详见 FRAME_SYNC §7.2） |
| `C2G_ClockSyncRequest` | C→S | MemoryPack | 每 10s | 时钟同步请求（详见 FRAME_SYNC §6.1） |
| `G2C_ClockSyncResponse` | S→C | MemoryPack | 每 10s | 时钟同步响应（详见 FRAME_SYNC §6.1） |
| `G2C_SpectatorBatch` | S→C | MemoryPack | 30 Hz | 观战延迟帧批次（详见 FRAME_SYNC §6.4） |
| `C2G_ReconnectAbort` | C→S | ProtoBuf | 一次性 | 客户端取消重连（详见 FRAME_SYNC §7.2） |
| `G2C_ReconnectProgress` | S→C | ProtoBuf | 重连期间 | 重连进度通知（详见 FRAME_SYNC §7.2） |
| `C2G_SnapshotAck` | C→S | ProtoBuf | 1-3 次 | 快照接收确认（详见 FRAME_SYNC §7.2） |
| `G2C_NetworkWarning` | S→C | ProtoBuf | 触发时 | 慢速客户端警告（详见 FRAME_SYNC §6.3.3） |
| `C2G_ClientVersionReport` | C→S | ProtoBuf | 每次连接时上报（含重连） | 客户端版本上报（HotfixHash + HotfixSignature + AssetManifestHash + ProtocolVersion + BurstVersion，详见 §4.5.4） |
...
message InputPayload
{
    ...
    int16 MoveY;           // 定点数 ×100
    uint16 ActionFlags;    // 位标志（定稿见 协议与序列化规范.md §5）：Jump/Attack/Pickup/Drop/SelectNext/SelectPrev/UseConsumable/Dash + 保留位
    int16 AimAngle;        // 定点数 ×100，[0, 628] 表示 [0, 2π]
    // 注：SpellId 字段已删除（2026-07-07 收口，A4）。当前法术槽由 SelectedSpellSlot 推导，详见 协议与序列化规范.md
}
...
```protobuf
// Outer/OuterMessage.proto 追加
// 注：本文档采用显式字段编号风格（proto3 最佳实践，利于版本兼容）
// Protocol ProtoBuf: 客户端版本上报（每次建立连接时上报，含首次连接与重连，承载 §4.5.4 ClientVersionReport）
message C2G_ClientVersionReport // IMessage
{
    string HotfixHash = 1;          // SHA256(Hotfix.dll)
    string HotfixSignature = 2;     // 构建机私钥对 HotfixHash 的数字签名（详见 §9.3）
    string AssetManifestHash = 3;  // YooAsset 版本清单哈希
    uint32 ProtocolVersion = 4;     // 协议版本号
    uint32 BurstVersion = 5;        // Burst 编译器版本
}
```
...
| Room | 房间内 N 人（N=RoomConfig.PlayerCount） | 全文本 | ✅ |
| QuickChat | 对局内 N 人（N=RoomConfig.PlayerCount） | 预设短语 + 表情 | ✅（仅快捷） |
...
玩家点击"快速匹配"
  → 服务器 Matchmaker 收集请求
  → 凑齐 RoomConfig.PlayerCount 人 → 生成房间 → 直接进入 Battle 状态
...
玩家点击"排位赛"
  → 服务器按 MMR 匹配（ELO 算法）
  → 凑齐 RoomConfig.PlayerCount 人相近 MMR → 生成房间
...
public static class MmrCalculator
{
    ...
    {
        // rank: 1=冠军, N=末位（N=RoomConfig.PlayerCount，即 opponentMmrs.Length + 1）
        var expected = 0.0;
        ...
        var N = opponentMmrs.Length + 1; // 总人数（自己 + 对手）= RoomConfig.PlayerCount
        var actual = (N - rank) / (N - 1.0); // 1→1.0, N→0.0（通用公式）
...
| 随机数 | 服务器下发种子，使用确定性 PRNG `Xorshift128Plus`（详见 [帧同步Netcode设计.md](./帧同步Netcode设计.md) §8.2，Burst 兼容，跨平台一致） |
| 物理 | 不用 Unity Physics（非确定），全部自定义 2D 像素碰撞 |
| 时间步长 | 固定 `dt = 1/30`，不用 `Time.deltaTime` |

### 8.3 周期调和（5 Hz，环境层专用）

> 频率：每 6 逻辑帧 = 200ms = 5 Hz（非 10 Hz）。10 Hz 应为每 3 帧 100ms，本设计选 5 Hz 平衡带宽与一致性。
...
public class AntiCheatSystem : EntitySystem
{
    ...
    public bool ValidateInput(uint playerId, InputPayload input, long frame)
    {
        ...
        // 输入频率检查：同一帧或乱序帧拒绝（30Hz 下 dt 应 >= 1）
        // 注：player.LastInputFrame 初始化为 -1 以区分首帧（不可使用 long.MinValue：dt = frame - long.MinValue 在 unchecked 下回绕为负值，仍 < 1，首帧会被误判为乱序）
        var dt = frame - player.LastInputFrame;
        if (dt < 1) return false; // 同帧重复或乱序，拒绝

        // 输入字段范围检查：MoveX/MoveY 是输入向量分量（归一化 ×100），范围 [-100, 100]
        if (Math.Abs(input.MoveX) > 100 || Math.Abs(input.MoveY) > 100) return false;

        // 输入向量长度检查：归一化向量长度应 <= 100（防止斜向加速）
        var inputMagnitude = Math.Sqrt((long)input.MoveX * input.MoveX + (long)input.MoveY * input.MoveY);
        if (inputMagnitude > 100) return false;

        // 位移幅度检查（防瞬移）：用玩家实际位移校验，单位是像素
        // MaxSpeed 单位：像素/秒；30Hz 单步位移 = MaxSpeed / 30；1.5 倍容忍
        // 注意：此处检查的是玩家状态位移，不是 input 字段本身（input 是归一化向量，非像素位移）
        // config 通过 GameRoom.RoomConfigComponent.ArenaConfig 获取（服务器端 GameRoom 子实体在匹配建立时填充 RoomConfigComponent；客户端收到 G2C_MatchResult 后将 ArenaConfig 字段填充到本地 GameRoom 实体的 RoomConfigComponent 组件中）
        var maxMove = config.MaxSpeed * (Fix64)1 / 30 * (Fix64)1.5;
        var playerMoveDelta = Fix64Math.Length(player.Position - player.LastPosition);
        if (playerMoveDelta > maxMove) return false;
    ...
    public void CompareHashes(long frame)
    {
        ...
        // 通用多数派裁决公式：阈值 = ⌊PlayerCount / 2⌋ + 1
        // - N=2: 阈值=2（全票一致，任一不一致标记 desync）
        // - N=3: 阈值=2
        // - N=4: 阈值=3（与原 4 人硬编码一致）
        // - N=8: 阈值=5
        // 注：与 帧同步Netcode设计 §2.2.2 的环境层哈希（≥2 票，因区域级宽容）不同
        //     玩家层哈希为全局级，阈值更严格（⌊N/2⌋+1 票）
        // RoomConfig 通过 GameRoom.RoomConfigComponent 获取（详见 §4.4.1）
        var threshold = (RoomConfig.PlayerCount / 2) + 1;  // 过半数
        var maxCount = majority.Count();
        if (maxCount < threshold)
        {
            // 标记对局异常，进入仲裁流程（不直接踢人）
            FlagMatchAnomaly(frame, $"No clear majority (hash split, threshold={threshold}, got={maxCount})");
            return;
        }

        foreach (var minority in hashes.Where(h => h.Hash != majority.Key))
        {
...
> **机制选型**：本设计统一采用**数字签名（非对称）**机制，不使用 HMAC（HMAC 为对称密钥机制，无公钥/私钥之分，与"公钥预共享"概念混淆）。
>
> **关键约束**：客户端运行时**不持有私钥**（私钥仅构建机持有），因此不能用"客户端用私钥签名 nonce"的挑战-响应模式。改用**预签名 + 服务器记录防重放**方案。

```
客户端构建时（构建机持有私钥）:
  1. 编译 Hotfix.dll
  2. 计算哈希 H = SHA256(Hotfix.dll)
  3. 用项目私钥（RSA-2048 或 ECDSA-P256）对 H 计算数字签名 → signature
  4. signature 内置到 AOT 程序集（公钥 pubKey 也内置 AOT）

客户端连接服务器时（预签名验证，无挑战-响应）:
  1. 客户端启动时用 AOT 内置公钥 pubKey 验证本地 Hotfix.dll 哈希与 signature 是否匹配
  2. 客户端将 (Hotfix.dll 哈希 H, signature) 上报服务器
  3. 服务器用预共享的客户端公钥验签 signature 是否对 H 有效
  4. 服务器比对 H 是否在白名单中（防客户端使用旧版本 Hotfix.dll）
  5. 验签失败或 H 不在白名单 → 拒绝连接

防重放攻击:
  - 服务器为每个 (RoomId, PlayerId, H) 记录会话标记，同一 (RoomId, PlayerId, H) 元组在同一连接会话内只能上报一次
  - 客户端断线重连视为新会话（旧标记已在 socket close 时清除，见下条），可直接重新上报
  - 服务器在检测到 PlayerId 对应的 Fantasy Session 断开（socket close）时清除该玩家的旧会话标记；C2G_ReconnectRequest 本身不触发清标记，仅作为本次重连会话建立后的首个客户端请求

热更场景:
  1. 新版本 Hotfix.dll 用同一私钥生成新签名
  2. 服务器同步更新白名单（记录合法 H 列表）
  3. 客户端拉取新 Hotfix.dll + 新签名，AOT 用内置公钥验签通过后加载
  4. 对局中锁定版本，热更仅在非 Battle 状态下生效
```
...
> **状态（2026-07-06 校对）**：目录迁移与路径修正已完成，文档同步仍待处理（见 §10.3）。

### 10.1 目录迁移：Client/Unity → Client（已完成）
...
`Docs/开发智能体配置.md` 第 24-32 行的项目结构需更新：
- `Client/Unity/` → `Client/`
...
- `fantasy_quickstart.md` 的 Unity 版本号修正（2022.3 → 6000.3.19f1，详见 §2.3）
- `fantasy_quickstart.md` 路径引用修正（如有 `Client/Unity/` 引用，改为 `Client/`，详见 §10.1 受影响文件）
- `Server/Server.csproj` 或相关 NuGet 包配置：Fantasy-Net / NLog 包版本由 `*` 锁定为具体版本（详见 §2.3 第 3 项）
...
- [ ] 验证协议导出工具能正常生成到新路径
- [ ] 创建 `AOT.asmdef`、`AOT.asmdef` 等新程序集骨架
...
- [ ] 256×256 竞技场地形生成
- [ ] 边界死亡规则
- [ ] N 人出生点（N=RoomConfig.PlayerCount，位置由 ArenaConfig.Seed 确定性生成，最小间距 ≥ Width/4）
- [ ] 道具刷新表
- [ ] 结算画面
...
### 12.2 网络验收
- [ ] N 人（N=2/4/8 三档实测）30Hz 对局，ping < 100ms 下无 desync
- [ ] ping 200ms 下 jitter buffer 生效，无卡顿
- [ ] 断线 30s 内重连成功，状态恢复
- [ ] 状态哈希 60 帧比对，0 误报

### 12.3 玩法验收
- [ ] N 人乱斗（N=4 默认）从匹配到结算 < 8 分钟
- [ ] 道具 Telegraph 3 秒可见
- [ ] 边界死亡规则正确触发
- [ ] 排位赛 MMR 更新正确
...
- [ ] 房间内 N 人 `AssetManifestHash` 不一致 → 服务器拒绝开始对局
...
| N 人带宽过高（N=8 时最坏情况，8 玩家 × 30Hz × 64B/帧 = 15KB/s 上行） | 低 | 低 | MemoryPack 压缩 + 输入差分编码 |
